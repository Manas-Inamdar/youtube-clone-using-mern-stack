...
function Video() {
  ...
  const [likes, setLikes] = useState(videoData?.likes?.length || 0);
  ...
  useEffect(() => {
    if (videoData) {
      ...
      const token = sessionStorage.getItem('token') || localStorage.getItem('token');
      if (token && videoData.likes) {
        try {
          const payload = JSON.parse(atob(token.split('.')[1]));
          setLiked(videoData.likes.includes(payload._id));
        } catch {}
      }
    }
  }, [videoData]);
                  ...
                  <div>
                    <div className="border-b border-b-gray-100">
                      <ul className="-mb-px flex items-center gap-5 text-sm font-sm">
                        ...
                        <li>
                          <button
                            className={`inline-flex items-center gap-2 px-1 py-3 rounded ${liked ? "text-blue-500" : "text-gray-600"} hover:text-black`}
                            onClick={handleLike}
                            style={{ background: "none", border: "none", cursor: "pointer" }}
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" fill={liked ? "currentColor" : "none"} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">

...
function AllVideo() {
  ...
  useEffect(() => {
    ...
    const fetchVideos = async () => {
      try {
        setLoader(true)
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/videos/allUserVideo/${userdata._id}`);
  ...
  const handleDelete = async (videoId) => {
    if (confirm('Are you sure you want to delete this video?')) {
      ...
      try {
        setLoader(true)
        await axios.delete(`${process.env.REACT_APP_API_URL}/api/v1/videos/delete/${videoId}`);
